...
def create_orders(party, order_mapping, order_date=None):
  ...
  """
  prod4 = Product.objects.filter(id=4)
  credit_card = CreditCard(nick_name=ordered_by.email, card_number='4111111111111111',
  ...
  for order_request in order_mapping:
    # there's order
    item = LineItem(product=prod4, price_category=LineItem.PRICE_TYPE[13][0], quantity=2, frequency=order_request[2], total_price=order_request[1])
    item.save()
    # add item to cart
    ordered_by = User.objects.get(email="taster%d@gmail.com" % order_request[0])
    cart = Cart(user=ordered_by, receiver=ordered_by, party=party)
    cart.add(item)
    ...
    order = Order(ordered_by=ordered_by, receiver=ordered_by, cart=cart, 
...
def create_non_party_orders(after_party_orders):
  """
  ...
  for order_request in after_party_orders:
    ...
    if party.event_date > timezone.now() - timedelta(days=7):
