...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    if options['month']:
      filter_date = datetime.strptime(options['week'], "%m/%Y").date()
    else:
    ...
    for o in Order.objects.filter(order_date__range=[start_datetime, end_datetime]):
      ...
      for item in o.cart.items.all():
        ...
        if item.frequency != SubscriptionInfo.FREQUENCY_CHOICES[1][0]:
          ...
          if o.cart.party is None:
            # just a followup order of VIP's (Tier B)
            tier = 'B'
...
def qualify_tree(pro_comp, root_pro):
  ...
  if root_total_sales >= 10000 and
              num_active_pros >= 10 and
              num_advanced_pros >= 5 and
              num_elite_pros >= 1:
    pro_comp[root_pro.id]['level'] = ProLevel.PRO_LEVEL_CHOICES[4][0]
  elif root_total_sales >= 7800 and
              num_active_pros >= 6 and
              num_advanced_pros >= 2:
    pro_comp[root_pro.id]['level'] = ProLevel.PRO_LEVEL_CHOICES[3][0]
  elif root_total_sales >= 5000 and
              num_active_pros >= 3:
    pro_comp[root_pro.id]['level'] = ProLevel.PRO_LEVEL_CHOICES[2][0]
  elif root_total_sales >= 1500 and downline_total_sales >= 1200:
