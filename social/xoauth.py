...
def main(argv):
  ...
  if (options.generate_xoauth_string or options.test_imap_authentication or
    ...
    if not (ok_for_3_legged or ok_for_2_legged):
      ...
      #       '--xoauth_requestor_id')
      return
    print options.oauth_token, options.oauth_token_secret
    access_token = OAuthEntity(options.oauth_token, options.oauth_token_secret)
    print consumer.key, consumer.secret, access_token.key, access_token.secret, options.user, options.proto, options.xoauth_requestor_id, options.nonce, options.timestamp
    ...
    return base64.b64encode(xoauth_string)
...
def get_xoauth_token_min( user, token, secret ):
  consumer = OAuthEntity('anonymous', 'anonymous')
  access_token = OAuthEntity(token, secret)

  #print consumer.key, consumer.secret, access_token.key, access_token.secret, user 
  xoauth_string = GenerateXOauthString(
        consumer, access_token, user, 'imap',
        None, None, None)
